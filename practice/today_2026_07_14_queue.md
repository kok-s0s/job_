# 2026-07-14 练习实现：生产者消费者队列

对应练习：[2026-07-14：生产者消费者队列](/roadmap/daily/2026-07-14)
...
- `BlockingQueue` 用 `std::mutex` 保护内部 `std::queue<int>`。
- 消费者用 `std::condition_variable::wait(lock, predicate)` 阻塞等待。
- 生产者写入 1000 条消息后调用 `shutdown()`。
- 消费者在队列清空且停止后优雅退出。
- 最终消费数量为 1000，总和为 499500。

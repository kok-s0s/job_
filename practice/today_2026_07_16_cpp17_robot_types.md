# 2026-07-16 练习实现：C++17 机器人场景类型表达

对应练习：[2026-07-16：C++17 机器人场景类型表达](/roadmap/daily/2026-07-16)
...
- `std::optional<ImuSample>` 明确表达 IMU 数据可能缺失。
- `std::variant<StartTask, StopTask, FaultEvent>` 表达机器人事件的有限集合。
- `std::visit` 对不同事件做类型安全分发。
- `std::string_view` 从命令行中解析命令名，不拷贝原字符串。
- 程序验证 7 次 IMU 成功、3 次缺失、4 个事件计数正确、4 条命令解析正确。
